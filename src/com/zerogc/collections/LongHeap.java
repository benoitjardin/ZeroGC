...
public class LongHeap {
    ...
    public LongHeap() {
        this(LongHeap.class.getSimpleName(), INITIAL_CAPACITY, GROWTH_FACTOR);
    }

    public LongHeap(String name) {
        this(name, INITIAL_CAPACITY, GROWTH_FACTOR);
    }

    public LongHeap(String name, int initialCapacity) {
    	this(name, initialCapacity, GROWTH_FACTOR);
    }

    public LongHeap(String name, int initialCapacity, float growthFactor) {
...
*/
    ...
    public int removeEntry(int entry) {
        ...
        long lastKey = this.key[lastEntry];
     ...
     */
    private int bubleUp(int node, long key) {
        while (node != 0) {
            int parent = (node -1) >> 1;
     ...
     * @return the node where to store the key.
     */
    private int bubleDown(int node, long key) {
        int child;
        while ((child = (node << 1)+1) < this.size) {
            // Find smallest child
            long childKey = this.key[this.node[child]];
            if (child+1 < this.size) {
                long rightChildKey = this.key[this.node[child+1]];
    ...
    public static class EntryIterator {
    	...
    	public void init(LongHeap heap) {
    		this.heap = heap;
    		node = -1;
    		nextNode = 0;
    	}
