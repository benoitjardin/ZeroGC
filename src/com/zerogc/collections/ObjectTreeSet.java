...
public class ObjectTreeSet {
    ...
    private int size = 0;
    private int root = -1;
    private Comparator.ObjectComparator comparator = new Comparator.ObjectComparator(); 
    ...
    public ObjectTreeSet() {
        this(ObjectTreeSet.class.getSimpleName(), Store.INITIAL_CAPACITY, Store.GROWTH_FACTOR);
    }

    public ObjectTreeSet(String name) {
        this(name, Store.INITIAL_CAPACITY, Store.GROWTH_FACTOR);
    }

    public ObjectTreeSet(String name, int initialCapacity) {
        this(name, initialCapacity, Store.GROWTH_FACTOR);
    }

    public ObjectTreeSet(String name, int initialCapacity, float growthFactor) {
        this(name, new Store(name, initialCapacity, growthFactor));
    }

    public ObjectTreeSet(String name, Store store) {
        this.log = LogManager.getLogger(name);
        this.store = store;
    }
     ...
     * @return the entry of the key or {@code -1} if not found.
     */
    public int findFirst(Object key) {
        int x = root;
        int entry = -1;
        while (x != -1) {
     ...
     * @return the entry of the key that was inserted.
     */
    public int insertMulti(Object key) {
        int parent = -1;
        int entry = root;
        int cmp = 0;
    ...
    public static class EntryIterator {
        private ObjectTreeSet rbTree;
        private int entry;
        private int nextEntry;

        /** Initialize the iterator at the beginning of the collection. */
        public void init(ObjectTreeSet rbTree) {
            this.rbTree = rbTree;
            entry = -1;
