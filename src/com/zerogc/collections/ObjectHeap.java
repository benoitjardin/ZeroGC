...
*/
     ...
     */
    private int bubleUp(int node, Object key) {
        while (node != 0) {
            int parent = (node -1) >> 1;
     ...
     * @return the node where to store the key.
     */
    private int bubleDown(int node, Object key) {
        int child;
        while ((child = (node << 1)+1) < this.size) {
            // Find smallest child
            Object childKey = this.key[this.node[child]];
            if (child+1 < this.size) {
                Object rightChildKey = this.key[this.node[child+1]];
