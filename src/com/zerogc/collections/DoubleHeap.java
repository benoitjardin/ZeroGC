...
public class DoubleHeap {
    ...
    public DoubleHeap() {
        this(DoubleHeap.class.getSimpleName(), INITIAL_CAPACITY, GROWTH_FACTOR);
    }

    public DoubleHeap(String name) {
        this(name, INITIAL_CAPACITY, GROWTH_FACTOR);
    }

    public DoubleHeap(String name, int initialCapacity) {
        this(name, initialCapacity, GROWTH_FACTOR);
    }

    public DoubleHeap(String name, int initialCapacity, float growthFactor) {
...
*/
    ...
    public int removeEntry(int entry) {
        ...
        double lastKey = this.key[lastEntry];
     ...
     */
    private int bubleUp(int node, double key) {
        while (node != 0) {
            int parent = (node -1) >> 1;
     ...
     * @return the node where to store the key.
     */
    private int bubleDown(int node, double key) {
        int child;
        while ((child = (node << 1)+1) < this.size) {
            // Find smallest child
            double childKey = this.key[this.node[child]];
            if (child+1 < this.size) {
                double rightChildKey = this.key[this.node[child+1]];
