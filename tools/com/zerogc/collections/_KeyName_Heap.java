...
public class _KeyName_Heap {
    ...
    public _KeyName_Heap() {
        this(_KeyName_Heap.class.getSimpleName(), INITIAL_CAPACITY, GROWTH_FACTOR);
    }

    public _KeyName_Heap(String name) {
        this(name, INITIAL_CAPACITY, GROWTH_FACTOR);
    }

    public _KeyName_Heap(String name, int initialCapacity) {
        this(name, initialCapacity, GROWTH_FACTOR);
    }

    public _KeyName_Heap(String name, int initialCapacity, float growthFactor) {
...
*/
    ...
    public int removeEntry(int entry) {
        ...
        _KeyType_ lastKey = this.key[lastEntry];
     ...
     */
    private int bubleUp(int node, _KeyType_ key) {
        while (node != 0) {
            int parent = (node -1) >> 1;
     ...
     * @return the node where to store the key.
     */
    private int bubleDown(int node, _KeyType_ key) {
        int child;
        while ((child = (node << 1)+1) < this.size) {
            // Find smallest child
            _KeyType_ childKey = this.key[this.node[child]];
            if (child+1 < this.size) {
                _KeyType_ rightChildKey = this.key[this.node[child+1]];
